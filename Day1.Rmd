...
When you execute code within the notebook, the results appear beneath the code. It's an experience of coding environment and descriptive notes TWO-in-ONE.
...
R Markdown is an easy-to-write rich text format for creating dynamic documents and reports.
...
Use one hash: `# Heading 1`

Use two hash: `## Heading 2`

Use three hash: `### Header 3`
...
Only different by automatic-added mark-down options at beginning of document.
...
2. Load the library(or libraries) when you need to activate the feature(s) for your R projects/codes.  Use **library()** function to activate package. (This is required every time you need the package(s)).
...
Use **CMD + ENTER** (Mac) or **CTRL + ENTER** (Windows) to run line by line
```{r}
...
The following nested functions call is difficult to read

```{r}
text = "1234.56789"
round(sqrt(as.double(text)),2) # nested function calling starts at inner most function call. as.double() is called first. sqrt() is called second. round() is called last.
```
...
Instead, use piping like below to improve the readability.
...
summary(allowance) # numeric columns offers basic statistic information.  'Personal_Disability_Allowance' doesn't.
...
The process you do wrangling, you usually want to replace the old data set. 
...
Make sure you replace the allowance data set (Don't create new variable)
```{r}
print(graduates)
summary(graduates)
```
...
Suggestions: change the column name and fixed the number by times to 1000.
