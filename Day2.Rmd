...
* in the final conclusion, enhance the story telling of data analysis 
...
Like the picture above shows, in the stage of **Understand** (exploring the data sets), Transform, Visualize and Model are used in an iterative manner so as to get the best early understaning about the data sets.
...
If the built-in plotting tools are not enough for you, 
go for **ggplot2**. 
It  is the most popular data visualization for R.

ggplot2 is an open-source data visualization package for R. 
ggplot2 breaks up graphs into semantic components such as **scales** and **layers**.
...
**a coordinate system** 
+ 
**and geom—visual that represent data points**
...
We use **geom_point()** function that requires x and y value for each geom point.
...
In this case, it's widely called Scatter Plot.
It is useful to explore the relation of two variables.
...
`geom_smooth()` and `geom_point()` require very similar parameters.
...
We used points, lines and smooth above on the same data set and they provide very different message. 
...
It's usually for counting a limit set of values
...
Just some quick exploration. They might not be very useful.  
...
Use `filter()` to extract required rows
Use `group_by()` and `summarise()` to group and aggregate total headcount for both male and female
```{r ggplot line}
...
## facet_wrap()
facets are useful categorical variables.
It split your plot into subplot (a.k.a facets) that each display one subset of the data.

facet_wrap() lets you work with ONE extra variable (besides x and y).  
Each categorical value will used to produce to a sub plot.
We use 'LevelOfStudy' here.  Since there are FOUR distinct values, you will see FOUR sub-plots
```{r}
graduates %>% ggplot() +
  geom_point(mapping = aes(x=AcademicYear, y=Headcount)) +
...
Minimal design is the recent trend.  Especially true when many are using small device like mobile phone or tablet for day-to-day communication.
...
In this simulation data, you can strongly see the pattern with the help of simple scatter-plot.
...
`y = a1 + x * a2`
x and y are the variable from data set
a1 and a2 are parameters that can vary to capture different patterns.

Let's generate a random value of `a1` as **intercept** and `a2` as **slope**.
Here, we use **runinf()** to generate random uniform distributed number

**Note**: 
You might have to repeatively run a few times before you can see the visualized random model represented by a orange straight line.
```{r}
model = tibble(
...
![Elegant Graphics for Data Analysis](https://ggplot2-book.org/cover.jpg)
###
```{r}
...
A execellent ebook for R Data Science
* Basic Concept of Model
* Model Building
* Example on Model
A web based ebook at
...
![R Studio Cheatsheets](https://raw.githubusercontent.com/rstudio/cheatsheets/main/pngs/data-visualization.png)
```{r}
